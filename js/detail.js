...
const SiteDetail = (() => {
  ...
  function renderINTL(data) {
    ...
    // --- Weekly Performance KPI Row (PLC OEE source — matches MySPD) ---
    const weekly = data.weekly || {};
    html += '<div class="kpi-row">';
    const plcFpy = weekly.plc_fpy;
    html += kpiCard('Crossbelt FPY', plcFpy != null ? `${plcFpy}%` : '—', 'good diverts / total',
      plcFpy != null ? (plcFpy > 95 ? 'green' : plcFpy > 80 ? 'yellow' : 'red') : 'grey');
    const plcMhe = weekly.plc_mhe_defect;
    html += kpiCard('Sort MHE Defect', plcMhe != null ? `${plcMhe}%` : '—', 'FTD + gap + tracking',
      plcMhe != null ? (plcMhe < 3 ? 'green' : plcMhe < 6 ? 'yellow' : 'red') : 'grey');
    const plcScan = weekly.plc_scan_defect != null ? weekly.plc_scan_defect : weekly.scan_defect_pct;
    html += kpiCard('Crossbelt Scan NR%', plcScan != null ? `${plcScan}%` : '—', 'target <3%',
      plcScan != null ? (plcScan < 3 ? 'green' : plcScan < 5.5 ? 'yellow' : 'red') : 'grey');
    const sorterOte = weekly.sorter_ote;
    html += kpiCard('Sorter OTE', sorterOte != null ? `${sorterOte}%` : '—', 'excl. DTW downtime',
      sorterOte != null ? (sorterOte > 90 ? 'green' : sorterOte > 80 ? 'yellow' : 'red') : 'grey');
    const disabledCount = carriers.disabled || 0;
    html += kpiCard('Disabled Carriers', String(disabledCount), `of ${carrierCount} fleet`,
      disabledCount > 100 ? 'red' : disabledCount > 50 ? 'yellow' : 'green');
    const iobTrips = weekly.iob_trips;
    html += kpiCard('IOB Trips', iobTrips != null ? String(iobTrips) : '—',
      weekly.iob_downtime_min ? `${weekly.iob_downtime_min} min downtime` : 'this week',
      iobTrips != null ? (iobTrips > 10 ? 'red' : iobTrips > 3 ? 'yellow' : 'green') : 'grey');
    const estopEvents = weekly.estop_events;
    html += kpiCard('E-Stop Events', estopEvents != null ? String(estopEvents) : '—',
      weekly.estop_downtime_min ? `${weekly.estop_downtime_min} min downtime` : 'this week (excl DTW)',
      estopEvents != null ? (estopEvents > 10 ? 'red' : estopEvents > 3 ? 'yellow' : 'green') : 'grey');
    const chutesDown = (data.chutes || {}).locked_out + (data.chutes || {}).inhibited;
    html += kpiCard('Chutes Down', String(chutesDown || 0), `of 144 total`,
      chutesDown > 10 ? 'red' : chutesDown > 5 ? 'yellow' : 'green');
    html += '</div>';
